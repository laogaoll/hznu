...
class Tech extends React.Component {
  ...
  async componentDidMount() {
    if (isN(this.store.currUser)) {
      ...
      // 获取课程名称name和课程代码code，其中code数量为课程数量
      let r = await this.props.mainStore.post(urls.API_QRY_CLS, null)
      let s = await this.props.mainStore.post(urls.API_QRY_CLS_OLD,null);
      //console.log(s.data[0].code);
      this.setState({ loading: false, clsList:r.data, oldcode:s.data[0].code});
      // 查询code查询Web程序设计课程的全部信息
 ...
 //根据课程代码code查询课程的所有信息
  doSelCls =async(e)=>{
   ...
   // console.log(this.state);
    ...
    this.setState({ loading: false, clsDetail:r.data, code:e, batchE:batchE, tecList:r.tecList, expList:r.expList,term:r.data[0]?.term,
    bname:r.data[0]?.bname });
  }

  //导入历史课程
  ImportH =async(e)=>{
    let params = {code: e}
    let r = await this.props.mainStore.post(urls.API_GET_CLS_MAIN_OLD,params)
    let {batchE} = this.state
    batchE.addr = r.data[0]?.addr
    r.data.map((item,i)=>{
      item.web = (item.web===1)?true:false
    })
    this.setState({ loading: false, clsDetail:r.data, batchE:batchE})
  }
  doImportH = async()=>{
    let {oldcode} = this.state;
    this.ImportH(oldcode);
  }
  // 导入同类课程
  ImportS =async(e)=>{
    let params = {code:e}
    let r = await this.props.mainStore.post(urls.API_QRY_SCLS,params)
   // console.log(r);
    let {batchE} = this.state
    batchE.addr = r.data[0]?.addr
    r.data.map((item,i)=>{
  ...
  render() {
    //将在state中存储的课程信息取出
    let {clsList,clsDetail,funList,partList,selMenu,tecList,expList,term,bname} = this.state
    let cls = clsDetail[0]
    //console.log(cls);
          ...
          <div className="m-bd">
            <div className="m-tab_list">
              ...
              <>
                ...
                <div className="m-fun">
                  <Tooltip placement="right"title="导入您以前的一节课程">
                    <div className="m-item"  style={{'background':'#666','color':'#fff'}} onClick={this.doImportH} >导入历史课程</div>
                  </Tooltip>
                  <Tooltip placement="right"title="导入同名不同老师的课程">
                    <div className="m-item" style={{'background':'#666','color':'#fff'}}  onClick={this.doImportS}>导入同类课程</div>
                  </Tooltip>
                </div>
                <div className="m-fun">
